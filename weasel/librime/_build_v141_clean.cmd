...
rem Prepare thirdparty prefix and inject prebuilt deps (capnproto/glog/yaml-cpp)
...
xcopy /e /y "%RIME_ROOT%\deps\capnproto\install_static\include\*" "%RIME_ROOT%\thirdparty\include\" >nul
xcopy /y    "%RIME_ROOT%\deps\capnproto\install_static\lib\*.lib" "%RIME_ROOT%\thirdparty\lib\" >nul
xcopy /e /y "%RIME_ROOT%\deps\glog\install\include\*" "%RIME_ROOT%\thirdparty\include\" >nul
xcopy /y    "%RIME_ROOT%\deps\glog\install\lib\*.lib" "%RIME_ROOT%\thirdparty\lib\" >nul
xcopy /e /y "%RIME_ROOT%\deps\yaml-cpp\install\include\*" "%RIME_ROOT%\thirdparty\include\" >nul
xcopy /y    "%RIME_ROOT%\deps\yaml-cpp\install\lib\yaml-cpp.lib" "%RIME_ROOT%\thirdparty\lib\" >nul
...
"%CMAKE_EXE%" -S "%RIME_ROOT%\deps\marisa-trie" -B "%RIME_ROOT%\deps\marisa-trie\build-v141" -G "Visual Studio 17 2022" -T v141 -DCMAKE_CONFIGURATION_TYPES=Release -DCMAKE_INSTALL_PREFIX="%RIME_ROOT%\thirdparty" -DCMAKE_CXX_FLAGS_RELEASE="/MT /O2 /Ob2 /DNDEBUG /std:c++17" -DCMAKE_C_FLAGS_RELEASE="/MT /O2 /Ob2 /DNDEBUG"
if errorlevel 1 goto :e
...
rem 使用动态运行时库(/MD)匹配预编译的capnproto/glog/yaml-cpp库
"%CMAKE_EXE%" -S "%RIME_ROOT%" -B "%RIME_ROOT%\build-v141" -G "Visual Studio 17 2022" -T v141 -DCMAKE_CONFIGURATION_TYPES=Release -DBUILD_STATIC=ON -DBUILD_SHARED_LIBS=ON -DBUILD_TEST=OFF -DENABLE_LOGGING=ON -DBOOST_USE_CXX11=ON -DCMAKE_INSTALL_PREFIX="%RIME_ROOT%\dist" -DCMAKE_PREFIX_PATH="%RIME_ROOT%\thirdparty" -DCMAKE_CXX_STANDARD=17 -DCMAKE_CXX_FLAGS_RELEASE="/MD /O2 /Ob2 /DNDEBUG /DGLOG_NO_ABBREVIATED_SEVERITIES /std:c++17" -DCMAKE_C_FLAGS_RELEASE="/MD /O2 /Ob2 /DNDEBUG" -DBOOST_ROOT="F:\3.Projects\mancan_v1\boost_1_89_0" -DBOOST_LIBRARYDIR="F:\3.Projects\mancan_v1\boost_1_89_0\lib64-msvc-14.1"
if errorlevel 1 goto :e
