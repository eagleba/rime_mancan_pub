...
// Fix Windows min/max macro conflicts - must be before any includes
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
Distance EditDistanceCorrector::LevenshteinDistance(const std::string &s1, const std::string &s2) {
  ...
  for (auto x = column_start; x <= s2len; x++) {
    ...
    for (auto y = column_start; y <= s1len; y++) {
      ...
      column[y] = std::min(std::min(column[y] + 1, column[y - 1] + 1), 
                           last_diagonal + SubstCost(s1[y - 1], s2[x - 1]));
...
Distance EditDistanceCorrector::RestrictedDistance(const std::string& s1,
  ...
  for(size_t i = 1; i <= len1; ++i) {
    ...
    for(size_t j = 1; j <= len2; ++j) {
      d[index(i, j)] = std::min(std::min(d[index(i - 1, j)] + 2, d[index(i, j - 1)] + 2),
