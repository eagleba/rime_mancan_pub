...
echo ===== [%date% %time%] capnproto+yaml-cpp test start =====>> "%LOG%"
call :main >> "%LOG%" 2>&1
set "RC=%errorlevel%"
echo ===== [%date% %time%] capnproto+yaml-cpp test end rc=%RC% =====>> "%LOG%"
exit /b %RC%
...
rem Test capnproto -> thirdparty (static /MT, C++17)
echo Testing capnproto compilation with /MT...
...
rem Test yaml-cpp -> thirdparty (static /MT, C++17)
echo Testing yaml-cpp compilation with /MT...
...
echo capnproto+yaml-cpp test completed successfully.
exit /b 0
