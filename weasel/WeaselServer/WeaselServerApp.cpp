...
WeaselServerApp::WeaselServerApp()
    : m_handler(std::make_unique<RimeWithWeaselHandler>(&m_ui)),
      tray_icon(m_ui) {
  OutputDebugStringA("WeaselServerApp: Constructor started\n");
  // m_handler.reset(new RimeWithWeaselHandler(&m_ui));
  m_server.SetRequestHandler(m_handler.get());
  OutputDebugStringA("WeaselServerApp: Request handler set\n");
  SetupMenuHandlers();
  OutputDebugStringA("WeaselServerApp: Constructor completed\n");
}

WeaselServerApp::~WeaselServerApp() {}

int WeaselServerApp::Run() {
  OutputDebugStringA("WeaselServerApp::Run: Starting server\n");
  if (!m_server.Start()) {
    OutputDebugStringA("WeaselServerApp::Run: Server start failed\n");
    return -1;
  }
  OutputDebugStringA("WeaselServerApp::Run: Server started successfully\n");
